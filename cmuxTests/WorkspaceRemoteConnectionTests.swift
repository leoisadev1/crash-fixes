...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    func testCodexHookStopWithoutTurnRetiresSessionMonitorLeases() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        ...
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        let sessionId = "codex-session-stop-without-turn"
        ...
        let leaseURL = leaseDirectory.appendingPathComponent("lease-stop-without-turn.json", isDirectory: false)
        try """
        {
          "createdAt" : 1777107522,
          "leaseId" : "lease-stop-without-turn",
          "sessionId" : "\(sessionId)",
          "surfaceId" : "\(surfaceId)",
          "turnId" : "turn-from-prompt-submit",
        ...
        let hookInput = """
        {"session_id":"\(sessionId)","cwd":"\(root.path)","hook_event_name":"Stop","model":"gpt-5.5","permission_mode":"default","stop_hook_active":false,"last_assistant_message":"Done"}
        """
        let result = runProcess(
            executablePath: cliPath,
            arguments: ["hooks", "codex", "stop"],
        ...
        let lease = try XCTUnwrap(JSONSerialization.jsonObject(with: data, options: []) as? [String: Any])
        XCTAssertNotNil(lease["retiredAt"], "Stop without a turn id should retire active session monitor leases")
    }
